...
export default function AdminCreateAgencyPage() {
	...
	// Prepopulate contact fields from session when in self mode
	useEffect(() => {
		if (adminMode === "self" && sessionUser) {
			setFormData((prev) => ({
				...prev,
				contactName: sessionUser.name || "",
				contactEmail: sessionUser.email || "",
				contactRole: "AGENCY_ADMIN",
			}));
	...
	useEffect(() => {
		...
		if (session.user?.role !== "PLATFORM_ADMIN" && session.user?.role !== "SUPERADMIN") {
	...
	const toggleCheckbox = (
		field: "intakeMethods" | "followUpMethods",
		value: string,
	) => {
		setFormData((prev) => {
				...
				<div className="w-full max-w-2xl">
					<div className="bg-white rounded-lg shadow-lg p-8 border border-gray-200 text-center">
						...
						<p className="text-lg text-gray-700 mb-6">
							The agency has been created and is pending approval. Once approved, the
							primary contact will receive an email with instructions to set their
							password.
						</p>
			...
			<div className="w-full max-w-3xl mx-auto">
				...
				<div className="bg-blue-50 border-l-4 border-[#0B4F96] p-4 rounded-r-lg mb-6 flex items-start gap-3">
					...
					<div className="text-sm text-gray-700">
						...
						<p>
							You can create agencies on behalf of agency admins. After creation,
							approve the agency to send the primary contact a password setup email.
						</p>
					</div>
				</div>

				{/* Agency Admin Mode Toggle */}
				<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
					<h2 className="text-base font-semibold text-gray-900 mb-1">Who will administer this agency?</h2>
					<p className="text-sm text-gray-500 mb-4">
						Each agency has exactly one administrator. Choose whether you will manage it or invite someone else.
					</p>
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
						...
						>
							<div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center ${
								adminMode === 'self' ? 'border-[#0B4F96]' : 'border-gray-300'
							}`}>
								{adminMode === 'self' && <div className="w-2.5 h-2.5 rounded-full bg-[#0B4F96]" />}
							</div>
							<div>
								<div className="flex items-center gap-2 font-semibold text-gray-900 mb-0.5">
									<UserCheck className="w-4 h-4 text-[#0B4F96]" />
									I am the agency admin
								</div>
								<p className="text-xs text-gray-500">
									Link this agency to your account ({sessionUser?.email})
								</p>
							</div>
						</button>
						...
						>
							<div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center ${
								adminMode === 'invite' ? 'border-[#0B4F96]' : 'border-gray-300'
							}`}>
								{adminMode === 'invite' && <div className="w-2.5 h-2.5 rounded-full bg-[#0B4F96]" />}
							</div>
							<div>
								<div className="flex items-center gap-2 font-semibold text-gray-900 mb-0.5">
									<UserPlus className="w-4 h-4 text-[#0B4F96]" />
									Invite an agency admin
								</div>
								<p className="text-xs text-gray-500">
									Send a password setup email to the agency&apos;s designated administrator
								</p>
							</div>
						</button>
					</div>
				...
				<div className="bg-white rounded-lg shadow-lg p-8 border border-gray-200 mb-6">
					<form onSubmit={handleSubmit} className="space-y-8">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div>
									...
									<p className="mt-1 text-xs text-gray-500">
										Your state-issued agency license number
									</p>
								</div>
						...
						<div>
							...
							<p className="text-sm text-gray-500 mb-4">
								{adminMode === "self"
									? "This agency will be linked to your account."
									: "This person will receive a password setup email and have full admin access to the agency."}
							</p>
									...
									{!selfEditMode ? (
										<div className="flex items-center justify-between">
											<div className="flex items-center gap-3">
												<div className="w-10 h-10 rounded-full bg-[#0B4F96] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
													{(formData.contactName || sessionUser?.name || "A").charAt(0).toUpperCase()}
												</div>
												<div>
											...
											<div className="flex items-center gap-3">
												...
												<button
													type="button"
													onClick={() => setSelfEditMode(true)}
													className="text-gray-500 hover:text-gray-700 p-1.5 rounded hover:bg-white transition-colors"
													title="Edit"
												>
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
												<div>
													...
													<input
														...
														className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#0B4F96] focus:border-transparent"
														disabled={loading}
													/>
												</div>
												<div>
								...
								<div className="space-y-4">
									...
									<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
										<p className="text-sm text-blue-800">
											Once you approve the agency, this person will receive an email invitation with a link to set up their password. They will have full admin access to manage the agency&apos;s staff, credentials, referrals, and subscription. The invitation link expires in 24 hours.
										</p>
									</div>
								</div>
							)}
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									updateField("agencySize", e.target.value as AgencySize)
								}
								className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0B4F96] focus:border-transparent bg-white"
								disabled={loading}
							>
							...
							<div className="space-y-6">
								...
								<div>
									<label className="block text-sm font-semibold text-gray-700 mb-3">
										How does your agency receive intake requests?{" "}
								...
								<div>
									<label
										htmlFor="intakeTrackingDescription"
										className="block text-sm font-semibold text-gray-700 mb-2"
									>
										How do you track intake and follow-up?
									</label>
									<p className="text-sm text-gray-600 mb-3">
										Describe your tracking system (e.g., "Custom dashboard with
										spreadsheet views", "CRM system", "Manual tracking")
									</p>
									<textarea
										id="intakeTrackingDescription"
										value={formData.intakeTrackingDescription}
										onChange={(e) =>
											updateField("intakeTrackingDescription", e.target.value)
										}
										rows={4}
										className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0B4F96] focus:border-transparent"
								...
								<div>
									<label className="block text-sm font-semibold text-gray-700 mb-3">
										What methods do you use for follow-ups?{" "}
										<span className="text-red-500">*</span>
									</label>
									<p className="text-sm text-gray-600 mb-3">Select all that apply</p>
									<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
