...
export default function Header() {
	...
	// AGENCY_USER has agency access too (middleware allows /agency/* except settings/users)
	const hasAgencyAccess = isAgencyAdmin || isAgencyUser;
	...
	const showMyAgency = (hasAgencyAccess || adminHasAgency) && !agencyIsSuspended;
				...
				{isMenuOpen && (
					<div className="lg:hidden mt-4 pb-4">
						<nav className="flex flex-col space-y-4">
							...
							{/* Dashboard sub-nav (mobile) — credentials link for AGENCY_USER */}
							{isAgencyUser && pathname?.startsWith("/dashboard") && (
								<div className="pl-4 border-l-2 border-gray-200 mt-2">
									<div className="text-xs text-gray-500 mb-2 uppercase">Dashboard Menu</div>
									<Link
										href="/dashboard/credentials"
										className={`block hover:text-[#3da777] mb-3 ${pathname?.startsWith("/dashboard/credentials") ? "text-[#3da777] font-medium" : "text-gray-600"}`}
										onClick={() => setIsMenuOpen(false)}
									>
										Credentials
									</Link>
