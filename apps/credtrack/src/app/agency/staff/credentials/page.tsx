...
export default function AgencyStaffCredentialsPage() {
  const router = useRouter();
  const { data: session, status: sessionStatus } = useSession();

  const [records, setRecords] = useState<StaffRecord[]>([]);
  const [stats, setStats] = useState({ totalTracked: 0, totalCredentials: 0, pendingReview: 0 });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const recs = (data.records ?? []) as StaffRecord[];
      setRecords(recs);
      setStats({
        totalTracked: recs.length,
        totalCredentials: recs.reduce((s, r) => s + r.credentialCount, 0),
        pendingReview: recs.reduce((s, r) => s + r.pendingReview, 0),
      });
          ...
          {filteredRecords.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500">
                {records.length === 0
                  ? "No staff have credentials yet. Visit a staff member's profile to upload credentials."
