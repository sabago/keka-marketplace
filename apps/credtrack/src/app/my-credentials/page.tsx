...
const ADMIN_ROLES = ['AGENCY_ADMIN', 'SUPERADMIN', 'PLATFORM_ADMIN'];
...
export default function MyCredentialsPage() {
  ...
  const handleUploadRenewal = () => {
    router.push('/my-credentials/upload');
  };
  ...
  const getFilteredCredentials = () => {
    ...
    switch (filter) {
      case 'needs-action': filtered = data.needsAction; break;
      case 'active': filtered = filtered.filter((c) => c.status === 'ACTIVE' && c.isCompliant); break;
      case 'pending': filtered = filtered.filter((c) => c.reviewStatus === 'PENDING_REVIEW'); break;
      case 'expired': filtered = filtered.filter((c) => c.status === 'EXPIRED'); break;
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'name-asc': return a.documentType.name.localeCompare(b.documentType.name);
        case 'name-desc': return b.documentType.name.localeCompare(a.documentType.name);
        case 'expiration-asc':
        ...
        case 'upload-asc': return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
        case 'upload-desc': return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
        default: return 0;
      }
    });
    return filtered;
  ...
  const getUniqueDocumentTypes = () => {
    ...
    data.credentials.forEach((c) => {
      if (!types.has(c.documentType.id)) types.set(c.documentType.id, c.documentType);
    });
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {documentTypeFilter !== 'all' && (
                          <span className="px-2 py-1 bg-blue-100 text-blue-700 text-sm rounded-full flex items-center gap-1">
                            ...
                            <button onClick={() => setDocumentTypeFilter('all')} className="hover:text-blue-900">×</button>
                          </span>
                        )}
                        <button
                          onClick={() => { setSearchQuery(''); setDocumentTypeFilter('all'); }}
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'all' ? 'bg-[#0B4F96] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                    >
                    ...
                    {data.stats.needsActionCount > 0 && (
                      <button
                        onClick={() => setFilter('needs-action')}
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'needs-action' ? 'bg-red-600 text-white' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                      >
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'active' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}
                    >
                    ...
                    {data.stats.pendingReview > 0 && (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'pending' ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}
                      >
                    ...
                    {data.stats.expired > 0 && (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'expired' ? 'bg-red-600 text-white' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}
                      >
