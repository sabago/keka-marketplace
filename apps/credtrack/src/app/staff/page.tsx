...
function AddStaffModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ firstName: "", lastName: "", email: "", position: "", department: "", hireDate: "" });
      onSuccess();
    } catch (err: any) {
      ...
      <button
        onClick={() => {
          if (!canAdd) {
            alert(
              staffLimit === -1
                ? "Unable to add staff at this time."
                : `You've reached your staff limit of ${staffLimit}. Upgrade your plan to add more.`
            );
            return;
          }
          setOpen(true);
        }}
        className="flex items-center gap-2 bg-[#0B4F96] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#0a4280] transition-colors"
      >
            ...
            <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 text-sm bg-[#0B4F96] text-white rounded-lg hover:bg-[#0a4280] disabled:opacity-50"
                >
...
export default function StaffPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [staffRes, typesRes] = await Promise.all([
        ...
        documentTypes.length === 0 ? fetch("/api/document-types") : Promise.resolve(null),
      ]);
      if (!staffRes.ok) throw new Error("Failed to load staff");
      const data = await staffRes.json();
      setRecords(data.records);

      // Staff limit is encoded in the session plan; derive it client-side
      const plan = session?.user?.credtrackPlan as CredTrackPlan | undefined;
      const limit = plan === "STARTER" ? 5 : -1; // STARTER cap matches credtrackSubscription.ts
      const current = (data.records as StaffRecord[]).length;
      setStaffLimit(limit);
      setCanAdd(limit === -1 || current < limit);
  ...
  const stats = {
    ...
    totalCredentials: records.reduce((s, r) => s + r.credentialCount, 0),
    pendingReview: records.reduce((s, r) => s + r.pendingReview, 0),
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-3">
            {staffLimit !== -1 && (
              <span className="text-sm text-gray-500">
                {records.length} / {staffLimit} staff
              </span>
            )}
            <AddStaffModal
              canAdd={canAdd}
                  ...
                  <div key={r.id} className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-gray-900">{r.firstName} {r.lastName}</div>
                          <div className="text-xs text-gray-500">{r.position || r.email || ""}</div>
                        </div>
                      </div>
                    </div>
      ...
      {uploadForRecord && (
        <DocumentUpload
          staffRecordId={uploadForRecord.id}
