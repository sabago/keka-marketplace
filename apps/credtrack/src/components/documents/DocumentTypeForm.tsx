...
export default function DocumentTypeForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to save document type"
      );
    } finally {
      setLoading(false);
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Days until this document type typically expires. Leave empty if
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              min="1"
              placeholder="e.g., 30 for monthly rechecks"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0B4F96]"
            />
            <p className="mt-1 text-xs text-gray-500">
              How often this document must be re-verified. Leave empty if a
              one-time upload is sufficient.
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-gray-500">
              Comma-separated days before expiration (e.g., &quot;30, 7&quot;).
            </p>
          </div>
          ...
          <fieldset className="border border-gray-200 rounded-lg p-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <label
                htmlFor="allowsMultiPage"
                className="text-sm text-gray-700"
              >
                Allow multi-page uploads
              </label>
            </div>
            ...
            {formData.requiresFrontBack && (
              <p className="text-xs text-gray-500">
                Front & back mode requires exactly 2 files (enforced
                automatically).
              </p>
            )}
          </fieldset>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mb-3">
              Additional metadata fields to collect at upload time.
            </p>
                ...
                {customFieldRows.map((row, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <input
                      ...
                      value={row.key}
                      onChange={(e) =>
                        updateCustomFieldKey(idx, e.target.value)
                      }
