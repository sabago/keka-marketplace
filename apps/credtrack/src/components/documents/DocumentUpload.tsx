...
interface DocumentUploadProps {
  ...
  inline?: boolean;
  /** When false, shows an AI parsing notice instead of silently skipping */
  canUseAI?: boolean;
}
...
export default function DocumentUpload({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CredTrack uses staffMemberId (not staffRecordId)
      formData.append("staffMemberId", staffRecordId);
      ...
      <form onSubmit={handleSubmit} className={inline ? "space-y-6" : "p-6 space-y-6"}>
          ...
          {!canUseAI && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800">AI parsing available on Growth plan</p>
                <p className="text-xs text-amber-700 mt-0.5">
                  Your document will be saved, but fields will not be auto-extracted. Upgrade to Growth or Enterprise to enable AI parsing.
                </p>
              </div>
            </div>
          )}
